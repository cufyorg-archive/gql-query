...
export function formatGraphqlArgument(arg) {
    ...
    if (Array.isArray(arg))
        return `[${arg.map(formatGraphqlArgument).join(',')}]`;
    return `{${Object.entries(arg).map(([name, value]) => `${name}:${formatGraphqlArgument(value)}`).join(' ')}}`;
}
/**
 * Compose a new graphql query arguments from the given args object.
 */
export function formatGraphqlArguments(args) {
    // runtime type check
    ...
    return `(${Object.entries(args).map(([name, value]) => `${name}:${formatGraphqlArgument(value)}`).join(' ')})`;
}
/**
 * Compose a new graphql query from the given fields.
 */
export function formatGraphqlQuery(fields) {
    // runtime type check

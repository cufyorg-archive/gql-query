 ...
 */
export function formatGraphqlArgument(argument) {
    if (!argument)
        // argument: null
        return 'null';
    ...
    return `{${Object.entries(argument).map(([name, value]) => name.startsWith('$') ?
        // value: any
 ...
 */
export function formatGraphqlArguments(_arguments) {
    if (!Object.keys(_arguments).length)
        return '';
    return `(${Object.entries(_arguments).map(([name, value]) => name.startsWith('$') ?
        // value: any
        `${name.substring(1)}:${value}` :
        // value: GraphqlArgument
        `${name}:${formatGraphqlArgument(value)}`).join(' ')})`;
